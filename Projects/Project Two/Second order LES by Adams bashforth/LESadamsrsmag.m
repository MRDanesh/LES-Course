...
file_u  = fopen([outpath,'Smagsecond_',int2str(Nx),'_U.out'],'w');
...
for s = 1:NSteps
    ...
    M11(1)=-2*h*h*(4*filter_Boundary(abs(grad_u(Nx,u,h,Nx)),abs(grad_u(1,u,h,Nx)),abs(grad_u(2,u,h,Nx)))*...
            filter_Boundary((grad_u(Nx,u,h,Nx)),(grad_u(1,u,h,Nx)),(grad_u(2,u,h,Nx)))-...
            filter_Boundary(abs(grad_u(Nx,u,h,Nx))*grad_u(Nx,u,h,Nx),abs(grad_u(1,u,h,Nx))*grad_u(1,u,h,Nx),abs(grad_u(2,u,h,Nx))*grad_u(2,u,h,Nx)));
    ...
    M11(Nx)=-2*h*h*(4*filter_Boundary(abs(grad_u(1,u,h,Nx)),abs(grad_u(Nx,u,h,Nx)),abs(grad_u(Nx-1,u,h,Nx)))*...
            filter_Boundary((grad_u(1,u,h,Nx)),(grad_u(Nx,u,h,Nx)),(grad_u(Nx-1,u,h,Nx)))-...
            filter_Boundary(abs(grad_u(1,u,h,Nx))*grad_u(1,u,h,Nx),abs(grad_u(Nx,u,h,Nx))*grad_u(Nx,u,h,Nx),abs(grad_u(Nx-1,u,h,Nx))*grad_u(Nx-1,u,h,Nx)));
    ...
    for j=2:Nx-1
        M11(j)=-2*h*h*(4*filter_2(abs(grad_u(j-1,u,h,Nx)),abs(grad_u(j,u,h,Nx)),abs(grad_u(j+1,u,h,Nx)))*...
            filter_2((grad_u(j-1,u,h,Nx)),(grad_u(j,u,h,Nx)),(grad_u(j+1,u,h,Nx)))-...
            filter_2(abs(grad_u(j-1,u,h,Nx))*grad_u(j-1,u,h,Nx),abs(grad_u(j,u,h,Nx))*grad_u(j,u,h,Nx),abs(grad_u(j+1,u,h,Nx))*grad_u(j+1,u,h,Nx)));
    ...
    for j=2:Nx-1
    tau(j)=-2*Cs(s)*h*h*abs((u(j+1,1)-u(j-1,1) )/(2*h))*(u(j+1,1)-u(j-1,1) )/(2*h);
    end
    
    for j=1
    tau(j)=-2*Cs(s)*h*h*abs((u(j+1,1)-u(j-1+Nx,1) )/(2*h))*(u(j+1,1)-u(j-1+Nx,1) )/(2*h);
    end
    
    for j=Nx
    tau(j)=-2*Cs(s)*h*h*abs((u(j+1-Nx,1)-u(j-1,1) )/(2*h))*(u(j+1-Nx,1)-u(j-1,1) )/(2*h);
    end
