...
file_u  = fopen([outpath,'staticsecond_',int2str(Nx),'_U.out'],'w');
...
for s = 1:NSteps
    ...
    for j=2:Nx-1
    tau(j)=-2*Cs*h*h*abs((u(j+1,1)-u(j-1,1) )/(2*h))*(u(j+1,1)-u(j-1,1) )/(2*h);
    end
    
    for j=1
    tau(j)=-2*Cs*h*h*abs((u(j+1,1)-u(j-1+Nx,1) )/(2*h))*(u(j+1,1)-u(j-1+Nx,1) )/(2*h);
    end
    
    for j=Nx
    tau(j)=-2*Cs*h*h*abs((u(j+1-Nx,1)-u(j-1,1) )/(2*h))*(u(j+1-Nx,1)-u(j-1,1) )/(2*h);
    end
