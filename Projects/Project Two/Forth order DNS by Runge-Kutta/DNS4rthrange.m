...
Nx  = 8192; h = 2*pi/Nx; NSteps = 2e6; deltat = 1e-4; NDNS = 8192;
...
outpath = ['.',filesep,'DNS_',int2str(Nx),filesep];
[~,~,~] = mkdir(outpath);

file_u  = fopen([outpath,'DNS_',int2str(Nx),'_U.out'],'w');
...
for s = 1:NSteps
    ...
    for j=1
        ...
        conv1(j,1)=  (4/3)*( (u(j,1)+u(j+1,1))^2 - (u(j,1)+u(j-1+Nx,1))^2)/(8*h)-...
         ...
         f1(j,1)  = diff1(j,1) - conv1(j,1) + f(j,1);
    ...
    for j=1
        ...
        f2(j,1)  = diff2(j,1) - conv2(j,1) + f(j,1);
    ...
    for j=1
        ...
        f3(j,1)  = diff3(j,1) - conv3(j,1) + f(j,1);
    ...
    for j=1
        ...
        f4(j,1)  = diff4(j,1) - conv4(j,1) + f(j,1);
    end
    for j=2
