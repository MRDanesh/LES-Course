...
u1mean_grad_x(1:nj)=0;
u2mean_grad_x(1:nj)=0;
u3mean_grad_x(1:nj)=0;
v1mean_grad_x(1:nj)=0;
v2mean_grad_x(1:nj)=0;
v3mean_grad_x(1:nj)=0;
w1mean_grad_x(1:nj)=0;
w2mean_grad_x(1:nj)=0;
w3mean_grad_x(1:nj)=0;

for i=1:ni
for j=1:nj
for k=1:nk
    
    u1mean_grad_x(j)= u1mean_grad_x(j)+ u1_grad_x(i,j,k)/ni/nk;
    u2mean_grad_x(j)= u2mean_grad_x(j)+ u2_grad_x(i,j,k)/ni/nk;
    u3mean_grad_x(j)= u3mean_grad_x(j)+ u3_grad_x(i,j,k)/ni/nk;
    v1mean_grad_x(j)= v1mean_grad_x(j)+ v1_grad_x(i,j,k)/ni/nk;
    v2mean_grad_x(j)= v2mean_grad_x(j)+ v2_grad_x(i,j,k)/ni/nk;
    v3mean_grad_x(j)= v3mean_grad_x(j)+ v3_grad_x(i,j,k)/ni/nk;
    w1mean_grad_x(j)= w1mean_grad_x(j)+ w1_grad_x(i,j,k)/ni/nk;
    w2mean_grad_x(j)= w2mean_grad_x(j)+ w2_grad_x(i,j,k)/ni/nk;
    w3mean_grad_x(j)= w3mean_grad_x(j)+ w3_grad_x(i,j,k)/ni/nk;
...
u1mean_grad_y(1:nj)=0;
u2mean_grad_y(1:nj)=0;
u3mean_grad_y(1:nj)=0;
v1mean_grad_y(1:nj)=0;
v2mean_grad_y(1:nj)=0;
v3mean_grad_y(1:nj)=0;
w1mean_grad_y(1:nj)=0;
w2mean_grad_y(1:nj)=0;
w3mean_grad_y(1:nj)=0;

for i=1:ni
for j=1:nj
for k=1:nk
    
    u1mean_grad_y(j)= u1mean_grad_y(j)+ u1_grad_y(i,j,k)/nj/nk;
    u2mean_grad_y(j)= u2mean_grad_y(j)+ u2_grad_y(i,j,k)/ni/nk;
    u3mean_grad_y(j)= u3mean_grad_y(j)+ u3_grad_y(i,j,k)/ni/nk;
    v1mean_grad_y(j)= v1mean_grad_y(j)+ v1_grad_y(i,j,k)/ni/nk;
    v2mean_grad_y(j)= v2mean_grad_y(j)+ v2_grad_y(i,j,k)/ni/nk;
    v3mean_grad_y(j)= v3mean_grad_y(j)+ v3_grad_y(i,j,k)/ni/nk;
    w1mean_grad_y(j)= w1mean_grad_y(j)+ w1_grad_y(i,j,k)/ni/nk;
    w2mean_grad_y(j)= w2mean_grad_y(j)+ w2_grad_y(i,j,k)/ni/nk;
    w3mean_grad_y(j)= w3mean_grad_y(j)+ w3_grad_y(i,j,k)/ni/nk;
...
u1mean_grad_z(1:nj)=0;
u2mean_grad_z(1:nj)=0;
u3mean_grad_z(1:nj)=0;
v1mean_grad_z(1:nj)=0;
v2mean_grad_z(1:nj)=0;
v3mean_grad_z(1:nj)=0;
w1mean_grad_z(1:nj)=0;
w2mean_grad_z(1:nj)=0;
w3mean_grad_z(1:nj)=0;   

for i=1:ni
for j=1:nj
for k=1:nk
    
    u1mean_grad_z(j)= u1mean_grad_z(j)+ u1_grad_z(i,j,k)/nj/nk;
    u2mean_grad_z(j)= u2mean_grad_z(j)+ u2_grad_z(i,j,k)/ni/nk;
    u3mean_grad_z(j)= u3mean_grad_z(j)+ u3_grad_z(i,j,k)/ni/nk;
    v1mean_grad_z(j)= v1mean_grad_z(j)+ v1_grad_z(i,j,k)/ni/nk;
    v2mean_grad_z(j)= v2mean_grad_z(j)+ v2_grad_z(i,j,k)/ni/nk;
    v3mean_grad_z(j)= v3mean_grad_z(j)+ v3_grad_z(i,j,k)/ni/nk;
    w1mean_grad_z(j)= w1mean_grad_z(j)+ w1_grad_z(i,j,k)/ni/nk;
    w2mean_grad_z(j)= w2mean_grad_z(j)+ w2_grad_z(i,j,k)/ni/nk;
    w3mean_grad_z(j)= w3mean_grad_z(j)+ w3_grad_z(i,j,k)/ni/nk;
    
end
end
end

% now we can calculate 2<s:s>. in this code u_uT means double production of
% tensor of u and transpose of tensor of u. also u_u means double
% production of tensor of u and tensor of u
...
for j=1:nj
for k=1:nk
u_u_1(i,j,k) = u_u_1(i,j,k)+(u1_grad_x(i,j,k) * u1_grad_x(i,j,k) + u1_grad_y(i,j,k) * v1_grad_x(i,j,k) + u1_grad_z(i,j,k) * w1_grad_x(i,j,k) + v1_grad_x(i,j,k) * u1_grad_y(i,j,k) + v1_grad_y(i,j,k) * v1_grad_y(i,j,k) + v1_grad_z(i,j,k) * w1_grad_y(i,j,k) + w1_grad_x(i,j,k) * u1_grad_z(i,j,k) + w1_grad_y(i,j,k) * v1_grad_z(i,j,k) + w1_grad_z(i,j,k) * w1_grad_z(i,j,k));
u_u_2(i,j,k) = u_u_2(i,j,k)+(u2_grad_x(i,j,k) * u2_grad_x(i,j,k) + u2_grad_y(i,j,k) * v2_grad_x(i,j,k) + u2_grad_z(i,j,k) * w2_grad_x(i,j,k) + v2_grad_x(i,j,k) * u2_grad_y(i,j,k) + v2_grad_y(i,j,k) * v2_grad_y(i,j,k) + v2_grad_z(i,j,k) * w2_grad_y(i,j,k) + w2_grad_x(i,j,k) * u2_grad_z(i,j,k) + w2_grad_y(i,j,k) * v2_grad_z(i,j,k) + w2_grad_z(i,j,k) * w2_grad_z(i,j,k));
u_u_3(i,j,k) = u_u_3(i,j,k)+(u3_grad_x(i,j,k) * u3_grad_x(i,j,k) + u3_grad_y(i,j,k) * v3_grad_x(i,j,k) + u3_grad_z(i,j,k) * w3_grad_x(i,j,k) + v3_grad_x(i,j,k) * u3_grad_y(i,j,k) + v3_grad_y(i,j,k) * v3_grad_y(i,j,k) + v3_grad_z(i,j,k) * w3_grad_y(i,j,k) + w3_grad_x(i,j,k) * u3_grad_z(i,j,k) + w3_grad_y(i,j,k) * v3_grad_z(i,j,k) + w3_grad_z(i,j,k) * w3_grad_z(i,j,k));
u_uT_1(i,j,k)= u_uT_1(i,j,k)+ (u1_grad_x(i,j,k) * u1_grad_x(i,j,k) + u1_grad_y(i,j,k) * u1_grad_y(i,j,k) + u1_grad_z(i,j,k) * u1_grad_z(i,j,k) + v1_grad_x(i,j,k) * v1_grad_x(i,j,k) + v1_grad_y(i,j,k) * v1_grad_y(i,j,k) + v1_grad_z(i,j,k) * v1_grad_z(i,j,k) + w1_grad_x(i,j,k) * w1_grad_z(i,j,k) + w1_grad_y(i,j,k) * w1_grad_y(i,j,k) + w1_grad_z(i,j,k) * w1_grad_z(i,j,k));
u_uT_2(i,j,k)= u_uT_2(i,j,k)+ (u2_grad_x(i,j,k) * u2_grad_x(i,j,k) + u2_grad_y(i,j,k) * u2_grad_y(i,j,k) + u2_grad_z(i,j,k) * u2_grad_z(i,j,k) + v2_grad_x(i,j,k) * v2_grad_x(i,j,k) + v2_grad_y(i,j,k) * v2_grad_y(i,j,k) + v2_grad_z(i,j,k) * v2_grad_z(i,j,k) + w2_grad_x(i,j,k) * w2_grad_z(i,j,k) + w2_grad_y(i,j,k) * w2_grad_y(i,j,k) + w2_grad_z(i,j,k) * w2_grad_z(i,j,k));
u_uT_3(i,j,k)= u_uT_3(i,j,k)+ (u3_grad_x(i,j,k) * u3_grad_x(i,j,k) + u3_grad_y(i,j,k) * u3_grad_y(i,j,k) + u3_grad_z(i,j,k) * u3_grad_z(i,j,k) + v3_grad_x(i,j,k) * v3_grad_x(i,j,k) + v3_grad_y(i,j,k) * v3_grad_y(i,j,k) + v3_grad_z(i,j,k) * v3_grad_z(i,j,k) + w3_grad_x(i,j,k) * w3_grad_z(i,j,k) + w3_grad_y(i,j,k) * w3_grad_y(i,j,k) + w3_grad_z(i,j,k) * w3_grad_z(i,j,k));
end
end
end
...
pseudo_dissipation(1:nj)=0;
for i=1:ni
for j=1:nj
for k=1:nk
   
   pseudo_dissipation(j)= pseudo_dissipation(j)+ nu*(u_uT_1(i,j,k)+u_uT_2(i,j,k)+u_uT_3(i,j,k))/ni/nk/3;
   
end
end
end

%plot(yplus,pseudo_dissipation);

mean_disipation(1:nj)=0;

for j=1:nj
mean_disipation(j)= mean_disipation(j)+ nu*umean_grad(j)*umean_grad(j);
end

%plot(yplus,mean_disipation);

P_epsilon(1:nj)=0;
for j=1:nj
    P_epsilon(j)=P_epsilon(j)+ (Production(j)/disipation_rate(j));

end

%plot(yplus,P_epsilon);
xlabel('y+')
ylabel('Production/dissipation')



S_K_epsilon(1:nj)=0;
for j=1:nj
    S_K_epsilon(j)=S_K_epsilon(j)+ (umean_grad(j)* K(j)/disipation_rate(j));
end

plot(yplus,S_K_epsilon);
xlabel('y+')
ylabel('S*K/dissipation')
