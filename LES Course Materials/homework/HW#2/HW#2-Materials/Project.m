...
%plot(umeanj,y,'--','LineWidth',1);
%hold on;
%plot(u_3d_y,y,':','LineWidth',1.5);
%hold on;
%plot(u_filter_2n_y,y,'-','LineWidth',1 );
%hold on;
%plot(u_double_filter_2n_y,y,'-.','LineWidth',1);
%hold on;
%xlabel('y')
%ylabel('Velocity')
...
u1filter_grad_y(1:ni,1:nj,1:nk)=0;
u2filter_grad_y(1:ni,1:nj,1:nk)=0;
u3filter_grad_y(1:ni,1:nj,1:nk)=0;

for i=1:ni
for j=1:nj-1
for k=1:nk
  
u1filter_grad_y(i,j,k) = u1filter_grad_y(i,j,k)+ (u1_filter_2n(i,j+1,k)-u1_filter_2n(i,j,k))/(y(j+1)-y(j));
u2filter_grad_y(i,j,k) = u2filter_grad_y(i,j,k)+ (u2_filter_2n(i,j+1,k)-u2_filter_2n(i,j,k))/(y(j+1)-y(j));
u3filter_grad_y(i,j,k) = u3filter_grad_y(i,j,k)+ (u3_filter_2n(i,j+1,k)-u3_filter_2n(i,j,k))/(y(j+1)-y(j));
...
u1filter_grad_y(i,j,k) = 0;
u2filter_grad_y(i,j,k) = 0;
u3filter_grad_y(i,j,k) = 0;
...
u1filter_grad_x(1:ni,1:nj,1:nk)=0;
u2filter_grad_x(1:ni,1:nj,1:nk)=0;
u3filter_grad_x(1:ni,1:nj,1:nk)=0;
...
u1filter_grad_x(i,j,k) = u1filter_grad_x(i,j,k)+ (u1_filter_2n(i+1,j,k)-u1_filter_2n(i,j,k))/(dx);
u2filter_grad_x(i,j,k) = u2filter_grad_x(i,j,k)+ (u2_filter_2n(i+1,j,k)-u2_filter_2n(i,j,k))/(dx);
u3filter_grad_x(i,j,k) = u3filter_grad_x(i,j,k)+ (u3_filter_2n(i+1,j,k)-u3_filter_2n(i,j,k))/(dx);    
...
u1filter_grad_x(i,j,k) = u1filter_grad_x(i-1,j,k);
u2filter_grad_x(i,j,k) = u1filter_grad_x(i-1,j,k);
u3filter_grad_x(i,j,k) = u1filter_grad_x(i-1,j,k);
...
u1filter_grad_z(1:ni,1:nj,1:nk)=0;
u2filter_grad_z(1:ni,1:nj,1:nk)=0;
u3filter_grad_z(1:ni,1:nj,1:nk)=0;
...
u1filter_grad_z(i,j,k) = u1filter_grad_z(i,j,k)+ (u1_filter_2n(i,j,k+1)-u1_filter_2n(i,j,k))/(dz);
u2filter_grad_z(i,j,k) = u2filter_grad_z(i,j,k)+ (u2_filter_2n(i,j,k+1)-u2_filter_2n(i,j,k))/(dz);
u3filter_grad_z(i,j,k) = u3filter_grad_z(i,j,k)+ (u3_filter_2n(i,j,k+1)-u3_filter_2n(i,j,k))/(dz);    
...
u1filter_grad_z(i,j,k) = u1filter_grad_z(i,j,k-1);
u2filter_grad_z(i,j,k) = u1filter_grad_z(i,j,k-1);
u3filter_grad_z(i,j,k) = u1filter_grad_z(i,j,k-1);
...
v1filter_grad_x(1:ni,1:nj,1:nk)=0;
v2filter_grad_x(1:ni,1:nj,1:nk)=0;
v3filter_grad_x(1:ni,1:nj,1:nk)=0;
...
v1filter_grad_x(i,j,k) = v1filter_grad_x(i,j,k)+ (v1_filter_2n(i+1,j,k)-v1_filter_2n(i,j,k))/(dx);
v2filter_grad_x(i,j,k) = v2filter_grad_x(i,j,k)+ (v2_filter_2n(i+1,j,k)-v2_filter_2n(i,j,k))/(dx);
v3filter_grad_x(i,j,k) = v3filter_grad_x(i,j,k)+ (v3_filter_2n(i+1,j,k)-v3_filter_2n(i,j,k))/(dx);    
...
v1filter_grad_x(i,j,k) = v1filter_grad_x(i-1,j,k);
v2filter_grad_x(i,j,k) = v1filter_grad_x(i-1,j,k);
v3filter_grad_x(i,j,k) = v1filter_grad_x(i-1,j,k);
...
v1filter_grad_y(1:ni,1:nj,1:nk)=0;
v2filter_grad_y(1:ni,1:nj,1:nk)=0;
v3filter_grad_y(1:ni,1:nj,1:nk)=0;

for i=1:ni
for j=1:nj-1
for k=1:nk
  
v1filter_grad_y(i,j,k) = v1filter_grad_y(i,j,k)+ (v1_filter_2n(i,j+1,k)-v1_filter_2n(i,j,k))/(y(j+1)-y(j));
v2filter_grad_y(i,j,k) = v2filter_grad_y(i,j,k)+ (v2_filter_2n(i,j+1,k)-v2_filter_2n(i,j,k))/(y(j+1)-y(j));
v3filter_grad_y(i,j,k) = v3filter_grad_y(i,j,k)+ (v3_filter_2n(i,j+1,k)-v3_filter_2n(i,j,k))/(y(j+1)-y(j));
...
v1filter_grad_y(i,j,k) = 0;
v2filter_grad_y(i,j,k) = 0;
v3filter_grad_y(i,j,k) = 0;
...
v1filter_grad_z(1:ni,1:nj,1:nk)=0;
v2filter_grad_z(1:ni,1:nj,1:nk)=0;
v3filter_grad_z(1:ni,1:nj,1:nk)=0;
...
v1filter_grad_z(i,j,k) = v1filter_grad_z(i,j,k)+ (v1_filter_2n(i,j,k+1)-v1_filter_2n(i,j,k))/(dz);
v2filter_grad_z(i,j,k) = v2filter_grad_z(i,j,k)+ (v2_filter_2n(i,j,k+1)-v2_filter_2n(i,j,k))/(dz);
v3filter_grad_z(i,j,k) = v3filter_grad_z(i,j,k)+ (v3_filter_2n(i,j,k+1)-v3_filter_2n(i,j,k))/(dz);    
...
v1filter_grad_z(i,j,k) = v1filter_grad_z(i,j,k-1);
v2filter_grad_z(i,j,k) = v1filter_grad_z(i,j,k-1);
v3filter_grad_z(i,j,k) = v1filter_grad_z(i,j,k-1);
...
w1filter_grad_x(1:ni,1:nj,1:nk)=0;
w2filter_grad_x(1:ni,1:nj,1:nk)=0;
w3filter_grad_x(1:ni,1:nj,1:nk)=0;
...
w1filter_grad_x(i,j,k) = w1filter_grad_x(i,j,k)+ (w1_filter_2n(i+1,j,k)-w1_filter_2n(i,j,k))/(dx);
w2filter_grad_x(i,j,k) = w2filter_grad_x(i,j,k)+ (w2_filter_2n(i+1,j,k)-w2_filter_2n(i,j,k))/(dx);
w3filter_grad_x(i,j,k) = w3filter_grad_x(i,j,k)+ (w3_filter_2n(i+1,j,k)-w3_filter_2n(i,j,k))/(dx);    
...
w1filter_grad_x(i,j,k) = w1filter_grad_x(i-1,j,k);
w2filter_grad_x(i,j,k) = w1filter_grad_x(i-1,j,k);
w3filter_grad_x(i,j,k) = w1filter_grad_x(i-1,j,k);
...
w1filter_grad_y(1:ni,1:nj,1:nk)=0;
w2filter_grad_y(1:ni,1:nj,1:nk)=0;
w3filter_grad_y(1:ni,1:nj,1:nk)=0;

for i=1:ni
for j=1:nj-1
for k=1:nk
  
w1filter_grad_y(i,j,k) = w1filter_grad_y(i,j,k)+ (w1_filter_2n(i,j+1,k)-w1_filter_2n(i,j,k))/(y(j+1)-y(j));
w2filter_grad_y(i,j,k) = w2filter_grad_y(i,j,k)+ (w2_filter_2n(i,j+1,k)-w2_filter_2n(i,j,k))/(y(j+1)-y(j));
w3filter_grad_y(i,j,k) = w3filter_grad_y(i,j,k)+ (w3_filter_2n(i,j+1,k)-w3_filter_2n(i,j,k))/(y(j+1)-y(j));
...
w1filter_grad_y(i,j,k) = 0;
w2filter_grad_y(i,j,k) = 0;
w3filter_grad_y(i,j,k) = 0;
...
w1filter_grad_z(1:ni,1:nj,1:nk)=0;
w2filter_grad_z(1:ni,1:nj,1:nk)=0;
w3filter_grad_z(1:ni,1:nj,1:nk)=0;
...
w1filter_grad_z(i,j,k) = w1filter_grad_z(i,j,k)+ (w1_filter_2n(i,j,k+1)-w1_filter_2n(i,j,k))/(dz);
w2filter_grad_z(i,j,k) = w2filter_grad_z(i,j,k)+ (w2_filter_2n(i,j,k+1)-w2_filter_2n(i,j,k))/(dz);
w3filter_grad_z(i,j,k) = w3filter_grad_z(i,j,k)+ (w3_filter_2n(i,j,k+1)-w3_filter_2n(i,j,k))/(dz);    
...
w1filter_grad_z(i,j,k) = w1filter_grad_z(i,j,k-1);
w2filter_grad_z(i,j,k) = w1filter_grad_z(i,j,k-1);
w3filter_grad_z(i,j,k) = w1filter_grad_z(i,j,k-1);
...
for i=1:ni
for j=1:nj
    S11_2n(i,j,k)=S11_2n(i,j,k)+((u1filter_grad_x(i,j,k)+ u2filter_grad_x(i,j,k) + u3filter_grad_x(i,j,k)))/3;
    S12_2n(i,j,k)=S12_2n(i,j,k)+(0.5*(u1filter_grad_y(i,j,k)+ u2filter_grad_y(i,j,k) + u3filter_grad_y(i,j,k)+v1filter_grad_x(i,j,k)+ v2filter_grad_x(i,j,k) + v3filter_grad_x(i,j,k)))/3;
    S13_2n(i,j,k)=S13_2n(i,j,k)+(0.5*(u1filter_grad_z(i,j,k)+ u2filter_grad_z(i,j,k) + u3filter_grad_z(i,j,k)+w1filter_grad_x(i,j,k)+ w2filter_grad_x(i,j,k) + w3filter_grad_x(i,j,k)))/3;
    S23_2n(i,j,k)=S23_2n(i,j,k)+(0.5*(v1filter_grad_z(i,j,k)+ v2filter_grad_z(i,j,k) + v3filter_grad_z(i,j,k)+w1filter_grad_y(i,j,k)+ w2filter_grad_y(i,j,k) + w3filter_grad_y(i,j,k)))/3;
    S22_2n(i,j,k)=S22_2n(i,j,k)+((v1filter_grad_y(i,j,k)+ v2filter_grad_y(i,j,k) + v3filter_grad_y(i,j,k)))/3;
    S33_2n(i,j,k)=S22_2n(i,j,k)+((w1filter_grad_z(i,j,k)+ w2filter_grad_z(i,j,k) + w3filter_grad_z(i,j,k)))/3;
...
S_bar(1:nj,i:nj,1:nk)=0;
for k=1:nk
for i=1:ni
for j=1:nj
    
    S_bar(i,j,k)= ( 2*S_mag_2n(i,j,k) )^(0.5);
...
%%%%%
 ...
 TR12_Smago_Static (1:nj)=0;
 for k=1:nk
 for i=1:ni
 for j=1:nj
     
 TR12_Smago_Static (j) = TR12_Smago_Static (j) -2*Cs_Static*((delta_bar(j))^2) *S_bar(i,j,k)*S12_2n(i,j,k)/ni/nk;
...
S_double_filter= f4(S_bar,y,dx,dz,ni,nj,nk);
...
%%%%% Calculating Mij%%%%%%%%
 ...
 for i=1:ni
 for j=1:nj
    A(i,j,k)= 2*((delta_bar(j))^(2))*S_bar(i,j,k)*S11_2n(i,j,k); 
    B(i,j,k)= 2*((delta_bar(j))^(2))*S_bar(i,j,k)*S12_2n(i,j,k);
    C(i,j,k)= 2*((delta_bar(j))^(2))*S_bar(i,j,k)*S13_2n(i,j,k);
    D(i,j,k)= 2*((delta_bar(j))^(2))*S_bar(i,j,k)*S22_2n(i,j,k);
    E(i,j,k)= 2*((delta_bar(j))^(2))*S_bar(i,j,k)*S23_2n(i,j,k);
    F(i,j,k)= 2*((delta_bar(j))^(2))*S_bar(i,j,k)*S33_2n(i,j,k);
 end
 end
 end
...
Cs_Dynamic(1:ni,1:nj,1:nk)=0;    
for i=1:ni
for j=1:nj
for k=1:nk    
    
    
    Cs_Dynamic(i,j,k)=( (Cs_numerator(i,j,k))/(Cs_denominator(i,j,k)));
...
TR12_Smago_Dynamic (1:nj)=0;
 ...
 TR12_Smago_Dynamic (j) = TR12_Smago_Dynamic (j) -2*Cs_Dynamic(i,j,k)*((delta_bar(j))^2) *S_bar(i,j,k)*S12_2n(i,j,k)/ni/nk;
 
 end
 end
 end    
    
 % plot(y, TR12_Smago_Static);
  %hold on;  
 ...
 for i=1:ni
 for j=1:nj
 Mu_t(j)=Mu_t(j)+(Cs_Dynamic(i,j,k)*delta_bar(j))*(Cs_Dynamic(i,j,k)*delta_bar(j))*S_bar(i,j,k)/ni/nk;
 end
 end
 end
...
Tr12_2n(i,j,k)=Tr12_2n(i,j,k)+((Tr12_1_2n(i,j,k)+Tr12_2_2n(i,j,k)+Tr12_3_2n(i,j,k)))/ni/nk/3;
...
for i=1:ni
for j=1:nj
TR33_2n(i,j,k)=TR33_2n(i,j,k)+((Tr33_1_2n(i,j,k)+Tr33_2_2n(i,j,k)+Tr33_3_2n(i,j,k)))/ni/nk/3;
end
end
end
...
for j=1:nj
    ...
    Production_exact (i,j,k) = -1*(Tr11_2n(i,j,k)*S11_2n(i,j,k)+Tr21_2n(i,j,k)*S21_2n(i,j,k)+...
...
for i=1:ni
for j=1:nj
Tr11_model(i,j,k)= -2*Cs_Dynamic(i,j,k)*(delta_bar(j))*(delta_bar(j))*S_bar(i,j,k)*S11_2n(i,j,k);
Tr12_model(i,j,k)= -2*Cs_Dynamic(i,j,k)*(delta_bar(j))*(delta_bar(j))*S_bar(i,j,k)*S12_2n(i,j,k);
Tr13_model(i,j,k)= -2*Cs_Dynamic(i,j,k)*(delta_bar(j))*(delta_bar(j))*S_bar(i,j,k)*S13_2n(i,j,k);
Tr21_model(i,j,k)= -2*Cs_Dynamic(i,j,k)*(delta_bar(j))*(delta_bar(j))*S_bar(i,j,k)*S21_2n(i,j,k);
Tr22_model(i,j,k)= -2*Cs_Dynamic(i,j,k)*(delta_bar(j))*(delta_bar(j))*S_bar(i,j,k)*S22_2n(i,j,k);
Tr23_model(i,j,k)= -2*Cs_Dynamic(i,j,k)*(delta_bar(j))*(delta_bar(j))*S_bar(i,j,k)*S23_2n(i,j,k);
Tr31_model(i,j,k)= -2*Cs_Dynamic(i,j,k)*(delta_bar(j))*(delta_bar(j))*S_bar(i,j,k)*S31_2n(i,j,k);
Tr32_model(i,j,k)= -2*Cs_Dynamic(i,j,k)*(delta_bar(j))*(delta_bar(j))*S_bar(i,j,k)*S32_2n(i,j,k);
Tr33_model(i,j,k)= -2*Cs_Dynamic(i,j,k)*(delta_bar(j))*(delta_bar(j))*S_bar(i,j,k)*S33_2n(i,j,k);
end
end
end
...
for j=1:nj
    ...
    Production_model (i,j,k) = -1*(Tr11_model(i,j,k)*S11_2n(i,j,k)+Tr21_model(i,j,k)*S21_2n(i,j,k)+...
        Tr31_model(i,j,k)*S31_2n(i,j,k)+Tr12_model(i,j,k)*S12_2n(i,j,k)+Tr22_model(i,j,k)*S22_2n(i,j,k)+...
        Tr32_model(i,j,k)*S32_2n(i,j,k)+Tr13_model(i,j,k)*S13_2n(i,j,k)+Tr23_model(i,j,k)*S23_2n(i,j,k)+...
        Tr33_model(i,j,k)*S33_2n(i,j,k));
...
x = 0:dx:95*dx;
[Y,X] = meshgrid(y,x);
Z = Production_exact (1:ni,1:nj,nk/2);
contourf(X,Y,Z,20);

%%%%%%%%%% Part 2 Question 4 %%%%%%%%%%%%%


Production_model_mean (1:nj)=0;
Production_exact_mean (1:nj)=0;

for k=1:nk
for i=1:ni
for j=1:nj
    
    Production_model_mean(j) = Production_model_mean(j)+ Production_model(i,j,k)/ni/nk;
    Production_exact_mean(j) = Production_exact_mean(j)+ Production_exact(i,j,k)/ni/nk;

end
end
end

%plot(y,Production_model_mean);
%hold on;
%plot(y,Production_exact_mean);
%hold on;
