...
for j=1:nj-1
for k=1:nk
    u1_grad_y(i,j,k)= (u1_fluc(i,j+1,k)-u1_fluc(i,j,k))/dy(j);
    u2_grad_y(i,j,k)= (u2_fluc(i,j+1,k)-u2_fluc(i,j,k))/dy(j);
    u3_grad_y(i,j,k)= (u3_fluc(i,j+1,k)-u3_fluc(i,j,k))/dy(j);
    v1_grad_y(i,j,k)= (v1_fluc(i,j+1,k)-v1_fluc(i,j,k))/dy(j);
    v2_grad_y(i,j,k)= (v2_fluc(i,j+1,k)-v2_fluc(i,j,k))/dy(j);
    v3_grad_y(i,j,k)= (v3_fluc(i,j+1,k)-v3_fluc(i,j,k))/dy(j);
    w1_grad_y(i,j,k)= (w1_fluc(i,j+1,k)-w1_fluc(i,j,k))/dy(j);
    w2_grad_y(i,j,k)= (w2_fluc(i,j+1,k)-w2_fluc(i,j,k))/dy(j);
    w3_grad_y(i,j,k)= (w3_fluc(i,j+1,k)-w3_fluc(i,j,k))/dy(j);
end
end
end
...
for i=1:ni
for k=1:nk
    u1_grad_y(i,nj,k)= u1_grad_y(i,nj-1,k);
    u2_grad_y(i,nj,k)= u2_grad_y(i,nj-1,k);
    u3_grad_y(i,nj,k)= u3_grad_y(i,nj-1,k);
    v1_grad_y(i,nj,k)= v1_grad_y(i,nj-1,k);
    v2_grad_y(i,nj,k)= v2_grad_y(i,nj-1,k);
    v3_grad_y(i,nj,k)= v3_grad_y(i,nj-1,k);
    w1_grad_y(i,nj,k)= w1_grad_y(i,nj-1,k);
    w2_grad_y(i,nj,k)= w2_grad_y(i,nj-1,k);
    w3_grad_y(i,nj,k)= w3_grad_y(i,nj-1,k);
end
end    
...
s_s (1:ni,1:nj,1:nk)=0;
for i=1:ni
for j=1:nj
for k=1:nk
s_s(i,j,k) = (u1_grad_x(i,j,k) * u1_grad_x(i,j,k) + u1_grad_y(i,j,k) * u2_grad_y(i,j,k) + u1_grad_z(i,j,k) * u3_grad_x(i,j,k) + u2_grad_x(i,j,k) * u1_grad_y(i,j,k) + u2_grad_y(i,j,k) * u2_grad_y(i,j,k) + u2_grad_z(i,j,k) * u3_grad_y(i,j,k) + u3_grad_x(i,j,k) * u1_grad_z(i,j,k) + u3_grad_y(i,j,k) * u2_grad_z(i,j,k) + u3_grad_z(i,j,k) * u3_grad_z(i,j,k));
end
end
end
...
for j=1:nj
for k=1:nk
s_s_mean(j) = s_s_mean(j)+ s_s(i,j,k)/ni/nk/3;
end
end
end
...
for j=1:nj
   ...
   disipation_rate (j)= disipation_rate(j)+ 2*nu*s_s_mean(j);
...
s_s_tilda(1:ni,1:nj,1:nk)=0;

for i=1:ni
for j=1:nj
for k=1:nk
s_s_tilda(i,j,k)= u1_grad_x(i,j,k)*u1_grad_x(i,j,k) + v1_grad_x(i,j,k)*v1_grad_x(i,j,k) + w1_grad_x(i,j,k)*w1_grad_x(i,j,k) + u1_grad_y(i,j,k)*u1_grad_y(i,j,k) + v1_grad_y(i,j,k)*v1_grad_y(i,j,k) + w1_grad_y(i,j,k)*w1_grad_y(i,j,k) + u1_grad_z(i,j,k)*u1_grad_z(i,j,k) + v1_grad_z(i,j,k)*v1_grad_z(i,j,k) + w1_grad_z(i,j,k)*w1_grad_z(i,j,k);
end
end
end

disipation_tilda (1:nj)=0;
for i=1:ni
for j=1:nj
for k=1:nk
disipation_tilda (j)= nu*s_s_tilda(i,j,k)/ni/nk;
end
end
end

plot(y,disipation_tilda);
